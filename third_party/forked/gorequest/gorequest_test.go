...
func TestGetSetCookie(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		c, err := r.Cookie("API-Cookie-ID")
		if err != nil {
			t.Error(err)
		}
		if c == nil {
			t.Error("Expected non-nil request Cookie 'API-Cookie-ID'")
		} else if c.Value != "api-cookie-value" {
			t.Errorf("Expected 'API-Cookie-ID' == %q; got %q", "api-cookie-value", c.Value)
		}
	}))
	defer ts.Close()

	New().Get(ts.URL).
		AddCookie(&http.Cookie{Name: "API-Cookie-ID", Value: "api-cookie-value"}).
		End()
}
